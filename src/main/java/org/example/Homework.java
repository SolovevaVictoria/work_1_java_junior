...
public class Homework {
    ...
    static int countPersons(List<Person> persons, int x, double d) {
        ...
        return (int)persons.stream().filter(P -> P.getAge() > x)
                .filter(P -> P.getSalary() > d).count();
    }
    ...
    static OptionalDouble averageSalary(List<Person> persons, String x) {
        // TODO: Реализовать метод
        return persons.stream().filter(P -> P.getDepartment().getName().contains(x))
                .mapToDouble(P -> P.getSalary()).average();
    }
    ...
    static List<Person> minSalaryPersons(List<Person> persons) {
        // TODO: Реализовать метод
        return persons.stream()
                .collect(Collectors.toMap(Person::getDepartment, person -> person,(p1,p2)->p1.getSalary()<p2.getSalary()? p1:p2))
